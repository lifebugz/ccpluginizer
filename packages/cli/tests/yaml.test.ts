...
describe("parseYamlFrontmatter: depth and edge cases", () => {
  ...
  test("a leading comment under a bare key does not corrupt the nested map", () => {
    const body = ["metadata:", "  # generated by tool", "  product: voice"].join("\n");
    const meta = parseYamlFrontmatter(body)["metadata"] as Record<string, unknown>;
    expect(meta["product"]).toBe("voice");
  });

  test("a full-line comment terminates a plain scalar instead of folding into it", () => {
    const body = ["description: hello", "  # trailing comment", "name: x"].join("\n");
    const out = parseYamlFrontmatter(body);
    expect(out["description"]).toBe("hello");
    expect(out["name"]).toBe("x");
  });

  test("a bracketed prose value like [Beta] tool for [iOS] stays a string", () => {
    expect(parseYamlFrontmatter("description: [Beta] tool for [iOS]\n")["description"]).toBe(
      "[Beta] tool for [iOS]",
    );
  });

  test("a uniformly indented root mapping is dedented, not dropped", () => {
    const fm = extractFrontmatter("---\n  name: foo\n  description: bar\n---\nbody\n");
    expect(fm?.["name"]).toBe("foo");
    expect(fm?.["description"]).toBe("bar");
  });
