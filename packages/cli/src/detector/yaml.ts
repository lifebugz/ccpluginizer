...
function collectPlainContinuation(
  ...
  for (; i < lines.length; i++) {
    const line = lines[i] ?? "";
    // A full-line comment terminates a plain scalar in real YAML — never fold it in.
...
/**
 * Strip the common leading indent: legal YAML may indent the entire root mapping
 * (the v0.1.1 parser trimmed every line), and without this the top-level loop would
 * skip every line as "stray over-indented" and silently drop the file.
 */
function dedentCommonIndent(lines: string[]): string[] {
...
function collectNested(
  ...
  let peek = start;
  // Skip blank AND comment lines: a leading `# generated by tool` under `metadata:`
  // must not turn the nested map into a folded garbage string.
...
function coerceYamlValue(raw: string): unknown {
  ...
  if (raw.startsWith("[") && raw.endsWith("]")) {
    ...
    if (inner === "") return [];
    // Only treat as a flow list when the brackets delimit the entire value:
    // "[Beta] tool for [iOS]" is prose, not a sequence.
